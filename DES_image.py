...
def encrypt(input, output):
    ...
    fileDim = INFILE.readline()
    maxSize = INFILE.readline()
    OUTPUTFILE.write(fileType.rstrip('\n') + '\n'.encode('utf-8'))
    OUTPUTFILE.write(fileDim.rstrip('\n') + '\n'.encode('utf-8'))
    OUTPUTFILE.write(maxSize.rstrip('\n') + '\n'.encode('utf-8'))

    while bv.more_to_read:
        bitvector = bv.read_bits_from_file(64)
        bitvector_length = len(bitvector)
        if bitvector_length > 0:
            if bitvector_length != 64:
                bitvector.pad_from_right(64 - bitvector_length)
            L, R = bitvector.divide_into_two()
            for round_key in round_keys:
                L, R = R, (substitute(R.permute(expansion_permutation) ^ round_key)).permute(pbox_permutation) ^ L
            final_bitvector = R + L
            final_bitvector.write_to_file(OUTPUTFILE)
    OUTPUTFILE.close()
    return
